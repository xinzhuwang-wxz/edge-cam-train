...
### 10.3 声音识别底座:Perch v2(Apache 2.0)—— 填掉 BirdNET 的非商用坑

音频本是**可选模块 + 晚融合**(plan §9):同窗「既见又闻」升置信、「闻而不见」补离框/夜间/遮挡——正好接上 §10.1 RealBirdID 的「很多图得靠鸟鸣才判得了」。原计划一直卡在 **BirdNET 权重 CC-BY-NC-SA(非商用 + 传染)→ 出不了货,只能自训规避**;**Perch v2(Google DeepMind,2025-08)把这个坑填了**:

| 项 | **Perch 2.0** | BirdNET(旧红线) |
|---|---|---|
| 许可 | **Apache 2.0,完全可商用** ✅ | CC-BY-NC-SA ❌ 出不了货 + 传染 |
| 能力 | 近 **15,000 物种(~10,000 鸟)** + 音频 embedding | ~6,500 类 |
| 架构 | EfficientNet-B3,embedding **~12M**、分类头 ~91M | EfficientNetB0-like |
| 训练数据 | Xeno-Canto / iNaturalist / Animal Sound Archive / FSD50k | — |
| 转换 | 已有社区 ONNX / TFLite | — |

💡 **怎么用(官方最佳实践)**:它给的是**强 embedding**,但 logits **未校准、稀有种可能不可靠** → 别直接吃它的 logits;**在 embedding 之上训一个轻量线性头**适配目标地区物种、用自己的数据调阈值。这跟 plan §9 原本「自训轻量频谱图分类器」的路一拍即合——只是底座从「零起步」换成「Perch 强 embedding」,**又干净(Apache)又强**。

💡 **上端注意**:Perch 是 **EfficientNet-B3(带 SE / swish)**,直接上 V85x INT8 NPU 有回退软算子风险(同 §8.1 视觉选型的理由)→ 端侧仍走 §9 的 NPU-safe 处理(op 实测,或用 Perch 当 teacher / embedding 源,蒸馏 / 训一个 Lite0 频谱图小头落端)。音频本就**独立并行、不与视觉争主 NPU 路**。
