...
| [04-V861-AWNN部署转换.md](04-V861-AWNN部署转换.md) | **离线转换链**：FP32 ONNX → AWNN docker（PTQ）→ `_ipu.param/.bin`（**非** ACUITY/`.nb`，见 [[ADR-0009]]） |
| [05-V861-真板部署.md](05-V861-真板部署.md) | **真板全过程**：自建带 NPU 的固件 → 烧录 → 上板 → 3 条优化结论 → **板端 INT8 ≡ 上游 FP32 等价性验证**（IoU 0.973 / 226ms）→ ★**连续推理的 P0 拦路虎**（NPU 内存不释放）。含 16 通道对齐铁律 |
