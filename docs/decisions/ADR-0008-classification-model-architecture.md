...
2. **端侧 backbone = 纯 CNN，bake-off 定档**（**更新 ADR-0005 决策 5：Lite0 → Lite4 档**）。候选：**EfficientNet-Lite4@256（主推/保守，同族 pipeline+ACUITY 复用、INT8 掉 0.2%）** / MobileNetV4-Conv-M@320（体积优 9.7MB）/ RepViT-M1.5@224（激进 82.5%）。命门精度接近时按**四维加权裁决**（NPU 算子成熟度 > INT8 掉点 > 体积/克制 > 工具链成熟度）。**避 transformer/attention/SE慎用/h-swish实测**（V861 只保证 CNN 算子）；INT8-only，权重压 ~10–15MB。排除 FastViT/MobileOne（Apple 非商用）、ConvNeXt-V2（CC-BY-NC）。
   ...
   > **更新（2026-07-07，用户定「性能优先、量化/部署优先级降」）**：四维裁决**重排为精度第一**——命门 top-1 主导，四维（NPU 算子/INT8 掉点/体积/工具链）降为**同分 tie-breaker**。**bake-off 往上扩两档**（V861 算力更足，~2.5–3G 实时预算）：**MNV4-Conv-L@256（~83.4%，上轮因内存降级，性能优先召回）** · **RepViT-M2.3@224–256（~83.7%，当前纯 CNN 天花板）**；原三候选（Lite4@256 / MNV4-M@320 / RepViT-M1.5@224）留作**效率参照下限**。**再叠分辨率扫（256→320→384）**——crop 已放大，细粒度升分辨率 ROI 常 > 换大 backbone（04/05）。**硬红线不动**：纯 CNN、INT8-only、避 transformer/attention（EfficientViT/混合架构仍出局，非性能优先能救）；**内存墙放松非取消**——V861 M3 128MB 减 AI-ISP ~64MB、还要同跑检测+编码，权重上限 ~15→~30MB（32MB 的 MNV4-L 吃掉半数余量是真风险，须上板实测）。data-centric（洗标/长尾）+ 分辨率与「换大 backbone」并列做消融，别只堆参数。
...
3. **类数 / 地域策略：路线 A 优先、B 兜底**（**细化 ADR-0005 决策 6**）。
   ...
   - **扩地域做法**（如北美 fast-follow）：用"旧地域+新地域"数据 **warm-start 重训并集模型**（拿旧地域 checkpoint 初始化，**非在旧模型上增量续训**——避灾难性遗忘 + 分类头扩容训崩），推理 mask 按用户位置收窄；OTA 下发新模型 + 新地理清单。端侧**始终一个模型**。ArcFace embedding 让云端检索兜底稀有/新种不重训，但端侧识别新地域种须端侧含该类 → 端侧仍走 warm-start 重训。

10. **分类粒度 = 种级（species）**：这是细分类器的存在理由——只分到科/大类则检测器（5 类）已够、无需细分类器；种级契合竞品（Bird Buddy 6900 种 / Merlin）。命门"层级可用率"**加权激励报种**（种 1.0 > 属 > 科 > bird），层级回退是**诚实兜底**（报不准种时给属/科，比竞品"认不准就 Mystery"更有信息量），**非降低粒度目标**；自信报错种（critical_error）最重罚。
