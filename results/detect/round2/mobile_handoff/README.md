...
| 文件 | 说明 |
|---|---|
| `nanodet_feeder5_mobile_416.param` / `.bin` | **ncnn 模型**（2.3MB）。sigmoid + 归一化**已焊进图**（喂 0-255 出概率）|
| `feeder_416.onnx` | **ONNX**（同权威模型，**焊入 sigmoid+归一化，接口同 ncnn**）。见 §11 |
| `feeder_416.fp32.tflite` / `.fp16.tflite` | **TFLite**（4.6M / 2.4M，**焊入，接口同 ncnn**）。见 §11 |
| `feeder_detector.h` / `.cpp` | **C++ 核心**（iOS/Android 共用，ncnn）。含 decode + NMS，对外 `detect()` |
| `feeder_detect_ref.py` | **Python 参考**（ncnn 版，输出 JSONL + 画框）|
| `feeder_detect_tflite.py` | **Python 参考**（tflite 版，**decode 与 ncnn 逐行相同**）|
| `labels.txt` | 类别名（行号=类别 id）|
| `demo_bird.jpg` · `demos_tflite/` | 跑通样例 · tflite 跑的 6 张 demo 画框结果 |
...
## 11. 三种模型格式（ncnn / onnx / tflite）—— 同一权威模型、**接口一致**

本包提供同一权威模型（部署点 `main`, NanoDet-Plus-m-416, 1.0x, 固定 test bird AP50 **85.0**）的三种格式。**三者都焊入了 sigmoid+归一化、接口完全一致**——喂 0-255 BGR、输出概率 `[3598,37]`、**同一套 decode/NMS**（§2 契约对三格式都成立）。换格式只换文件，decode 代码一行不改。

| 格式 | 文件 | runtime | 输入 | 输出 |
|---|---|---|---|---|
| **ncnn** | `nanodet_feeder5_mobile_416.{param,bin}` | ncnn | 0-255 BGR | 概率 `[3598,37]` |
| **onnx** | `feeder_416.onnx` | onnxruntime / 板端 | 0-255 BGR（NCHW）| 概率 `[1,3598,37]` |
| **tflite** | `feeder_416.fp32.tflite` / `.fp16.tflite` | TFLite | 0-255 BGR（NHWC）| 概率 `[1,3598,37]` |

**换 tflite 的动机**：省掉 ncnn 的 OpenMP 编译（Android 上 TFLite 是一等公民）。**因接口一致，decode/NMS 与 ncnn 逐行相同**——`feeder_detect_tflite.py` 与 `feeder_detect_ref.py` 的 decode 完全一样，只换 runtime。

**Python 跑法（验收）**：
```bash
pip install ai-edge-litert opencv-python numpy      # 或 tflite_runtime / tensorflow
FEEDER_MODEL_DIR=. python feeder_detect_tflite.py demo_bird.jpg fp32 0.4 0.5 out.jpg
```
`demos_tflite/` 是 tflite 跑 6 张 demo 的画框结果，可直接查看（与 ncnn 输出逐字一致）。
...
- 诚实说明：跨 runtime **非严格 bit-exact**（各自浮点/SIMD 实现不同），但 fp32 已对齐到 `1e-5`、三方检测框全一致——产品级完全等价。

**移动端集成 tflite**：
- **Android**：TFLite 一等公民（`org.tensorflow:tensorflow-lite` / LiteRT，可选 NNAPI/GPU delegate），无 ncnn 的 OpenMP 依赖。
- **iOS**：`TensorFlowLiteC` / LiteRT framework。
- **decode/NMS 与 ncnn 完全相同**（喂 0-255、输出概率）：`feeder_detector.cpp` 的 decode 可**直接复用**，只把"喂 ncnn"换成"喂 tflite interpreter"，其余（DFL softmax / 中心点无+0.5 / 坐标缩放裁剪 / 逐类 NMS）一字不改。
