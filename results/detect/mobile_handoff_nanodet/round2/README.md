...
python tflite/feeder_detect_tflite.py demos/demo_bird.jpg fp32 0.4 0.5 out.jpg
```
- **fp16 vs fp32**：`fp16`（2.4M）权重半精度、体积/内存减半、ARM 上更快、精度近无损（检测框 ±1px）——**移动端部署用它**；`fp32`（4.6M）最高精度、与 onnx 逐字一致——作精度对照/验收。
- **移动端集成**：Android 用 `org.tensorflow:tensorflow-lite` / LiteRT（可选 NNAPI/GPU delegate），iOS 用 `TensorFlowLiteC` / LiteRT framework，**无 OpenMP 依赖**。decode/NMS 见 `feeder_detect_tflite.py`（与 ncnn 版逐行相同），移植到 Kotlin/Swift 即可。
...
```
三份参考（`ncnn/feeder_detector.cpp` C++、`ncnn/feeder_detect_ref.py`、`tflite/feeder_detect_tflite.py`）decode **逐行相同**。
