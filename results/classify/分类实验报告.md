...
- **Katna**（可商用）：现成的**关键帧抽取**（去模糊 / 去重 / 选代表帧），连拍选帧可直接拿来用。
...
**库选型（商用口径）**：OpenCV（Apache，拉普拉斯方差打底）、**Katna**（关键帧）、MediaPipe（智能构图）均**可商用**；⚠️ **pyiqa 非商用**，只能做原型选型 / 对照，**不进发行**。整步纯端侧、纯打分、不引入大模型依赖，许可与项目红线（CLAUDE.md §4，避 AGPL/GPL/NC）一致。

---

## 📹 端侧门控/录制主线参考 · Frigate（MIT）

「**移动侦测唤醒 → 有鸟门控 → 升级触发 → 录片**」这条端侧主线，最该参考的成熟项目是 **Frigate**（MIT，部分可 fork）：开源**本地 NVR**，**实时本地物体检测、视频不上云**，靠 **Coral TPU** 等边缘加速，用 **MQTT** 触发事件做自动化 / 通知。核心理念正是我们要的——**「只回看相关移动（如人 / 鸟），不是风吹树叶」**：把检测下放到便宜的 Coral，树莓派级低成本硬件也能跑出好效果。

它的 **`motion → object detect → zone → event → record → clip → notify`** 管线，基本就是我们那套端侧门控**做成熟的样子**——可整段参考、按需 fork。

> ⚠️ **替我们记一个授权坑**：检测常用的 **Ultralytics YOLO 是 AGPL**（商用要单独授权），**YOLOv9 是 GPL**、与 Frigate 的 MIT 兼容。选检测 backbone 必须**避开 AGPL**——这也正是本项目用 **NanoDet（Apache）** 而非 Ultralytics 系的原因（CLAUDE.md §4 许可红线）。
...
## 🛰️ 可选的外部底座 · SpeciesNet / PyTorch-Wildlife（商用 + 选型角度）

两个 **Apache / MIT 许可**的现成资产，正好补上前面那些鸟图数据集「非商用」的坑。但要点先说死：**两者都是「云端模型」或「开发期脚手架」，没有一个是能塞进 V85x 的端侧模型。**

### SpeciesNet（google/cameratrapai，Apache 2.0）—— 云端大分类器底座

**是什么**：一个集成——检测用 **MegaDetector**（找动物/人/车，不分到种）＋ 分类器是 Google 用 **EfficientNet V2-M** 在 **6500 万张**相机陷阱图上训的，分 **2000+ 标签**（物种、更高阶 taxa 如「哺乳纲/猫科」、加「空帧」「车辆」非动物类），并可选用**地理信息**把每张图归到单一类别（不在该区域的种 roll-up 到更高 taxon，与本报告 §7 地域先验同源）。Google 公布（[TechCrunch](https://techcrunch.com/2025/03/03/google-releases-speciesnet-an-ai-model-designed-to-identify-wildlife/)，本报告未独立复核）：含动物图**检出 99.4%**、做种级判断时**准确率 94.5%**，且专门为**模糊和遮挡（尤其夜间红外）**优化。

**怎么用**：当「**云端大分类器 + 地理先验 + 空帧过滤**」那一层的**起点 + 微调底座**。边界——它是**相机陷阱/野生动物口径（2000 类）**，不是 6000–10000 种的细粒度鸟类：开箱即用适合「**有没有动物 / 粗到 taxon / 空帧过滤**」；要做细粒度**鸟种**，还得在它上面用**我们的回流数据微调**。但有个 Apache 强底座 + MegaDetector + 地理先验现成，比从零强太多。

**商用角度**：代码 Apache 2.0、明确允许商用；但权重托管 Kaggle 有自己条款，65M 训练数据 provenance 不透明 → 仍是「**输出授权灰区**」。**作为云端推理底座（Apache 权重、商用已授权）最干净**；若想**蒸馏/微调进可发行的端侧权重**，要走 [[ADR-0005]] 谨慎核（teacher 含 NC 数据会传染 student）。

**技术/部署角度 —— 能不能上 V85x（0.5T / 128MB，INT8-only）？不能上端，只能云端：**

| | 端侧分类器（现）Lite0 | SpeciesNet 分类器 EffNetV2-M |
|---|---|---|
| 参数量 | 3.8M | **~54M（~14×）** |
| INT8 权重 | 4.1MB | **~54MB** |
| 输入分辨率 | 224² | 384–480²（V2-M 常用，更大） |
| 关键算子 | 去 SE、swish→ReLU6（NPU 原生安全） | Fused-MBConv + **SE + SiLU**（Vivante VIP 大概率回退软算子） |
| 目标平台 | V85x 端侧 | 桌面 / 服务器 GPU |

三条硬约束都过不了：① **内存**——~54MB INT8 权重单这一项就吃掉 **128MB DDR 下限**近一半，还要塞 NanoDet + 激活 + 系统/app，0.5T/128MB 机型基本塞不下；② **算子**——V2-M 的 **SE / SiLU** 在 Vivante VIP NPU 上多半**回退软算子（A7 CPU）**，又慢又不划算（这正是我们选 **Lite0：去 SE、swish→ReLU6** 的原因）；③ **它默认捆的检测器是 MegaDetector v5（`md_v5a`）= YOLOv5 系**，带 **Focus/slice** + **AGPL**，同时踩中我们「忌 Focus/slice」「避 AGPL」两条红线（CLAUDE.md §4/§6）——好在我们用**自己的 NanoDet**，不照搬它的检测段。

### PyTorch-Wildlife（microsoft/CameraTraps，MIT）—— 开发期脚手架，不是端侧

**是什么**：微软 AI for Good 的**保护 AI 框架**——模型动物园（动物检测/分类的预训练权重）+ 推理/微调工具，相机陷阱口径。定位是**开发期脚手架**，不是部署到芯片的运行时。

**能不能用**：
- ✅ **开发期可用**——框架 **MIT**，干净；可当「拿动物检测/分类预训练权重 + 微调脚手架 + 数据工具」的现成件，**配合 SpeciesNet 在云端攒一条「回流数据微调」管线**。
- ⚠️ **商用注意**——框架 MIT，但它 wrap 的**模型权重各有许可**（它默认走 MegaDetector，v5=YOLOv5/AGPL）→ **别把这些预训练权重直接打进发行件**，按需取 MIT/Apache 变体（MDv6-mit/apa）。
- ❌ **不替换我们的核心栈**——我们已有 timm + Lightning + Hydra（分类）/ NanoDet（检测）在跑，按 §3 复用规则「借扩展点、不 fork 框架」，它当**可选参考/取权重**即可，不必改造成主训练器；**与 V85x 端侧部署无关**（纯 dev 工具）。

> **与既有判断一致**：`docs/classify/01 §2` 早把 SpeciesNet 标「服务器级、第一版不上端」——这里把两者的正确位置说清：**端侧 ❌；云端底座（SpeciesNet）/ 开发脚手架（PyTorch-Wildlife）✅**，且都是 Apache/MIT 可商用、用来补非商用数据坑的好牌。
