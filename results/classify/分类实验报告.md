...
> 月份 mask 同理（index 含 `observed_at`，可按「每种在场月份」建掩码），本轮未跑。
> 机读 `regional/regional_results.json`；清单 `regional/regions/*.json`；crosswalk `regional/ebird_map.csv`（347/360）。
...
**下一步**（按价值）：
1. **月份 mask**——index 含 `observed_at`，按「每种在场月份」建掩码、同 §7 口径跑（**地域已完成，见 §7**）。
...
| `regional/regional_results.json` · `regions/*.json` · `ebird_map.csv` | 地域 mask 结果 + 各国在场清单 + eBird crosswalk |
| `crop_dataset.py` · `cascade_demo.py` · `build_regions.py` · `regional_eval.py` · `make_classify_report_assets.py` | 复现脚本 |
| `figures/` | 本报告全部图 |
